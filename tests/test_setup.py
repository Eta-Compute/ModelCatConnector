...
from modelcatconnector.setup import run_setup, setup_cli


class TestSetup(unittest.TestCase):
    """Test cases for the setup module."""

    @patch('modelcatconnector.setup.check_awscli')
    @patch('modelcatconnector.setup.input')
    @patch('modelcatconnector.setup.getpass')
    @patch('modelcatconnector.setup.ProductAPIClient')
    @patch('modelcatconnector.setup.run_cli_command')
    @patch('modelcatconnector.setup.check_aws_configuration')
    @patch('modelcatconnector.utils.aws.check_s3_access')
    @patch('modelcatconnector.setup.os.makedirs')
    ...
    @patch('modelcatconnector.setup.run_setup')
    ...
    def test_run_setup_api_error(self, mock_api_client, mock_getpass, mock_input, mock_check_awscli):
        ...
        from modelcatconnector.utils.api import APIError
        mock_client_instance = MagicMock()
